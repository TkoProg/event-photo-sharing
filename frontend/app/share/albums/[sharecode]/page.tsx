...
const PREVODI = {
  BS: {
    ...
    pregledajte: 'Pregledajte fotografije sa ovog događaja.',
  ...
  EN: {
    ...
    pregledajte: 'Browse photos from this event.',
...
export default function ShareAlbumPage() {
  ...
  useEffect(() => {
    ...
    // TODO: Kada backend bude gotov, zamijeniti sa:
    // const res = await fetch(`http://127.0.0.1:8000/share/albums/${shareCode}`);
    // const data = await res.json();
 
    // Za sada čitamo iz localStorage — tražimo album sa ovim share kodom
    try {
      const savedAlbums: Album[] = JSON.parse(localStorage.getItem('moji_albumi') || '[]');
      const foundAlbum = savedAlbums.find(a => a.shareCode === shareCode);
 
      if (foundAlbum) {
        setAlbum(foundAlbum);
        // Učitaj slike albuma
        const albumPhotoUrls: string[] = JSON.parse(
          localStorage.getItem(`album_photos_${foundAlbum.id}`) || '[]'
        );
        // Pretvori URL-ove u Photo objekte
        const albumPhotos: Photo[] = albumPhotoUrls.map((url, index) => ({
          id: index + 1,
          url,
          tags: [],
        }));
      ...
      console.error('Greška pri učitavanju albuma:', err);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 pt-10 pb-6">
        ...
        <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">{album.name}</h1>
